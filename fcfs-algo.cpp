...
int ftime(int a,int s,int rem) {
    ...
    if((interval[st]>=s + (rem+rate[st]-1)/rate[st])) { 
        ...
        if(x>info[a][1]) {  // if end time of last interval
            return -1;
        }
        return x;
    }
...
signed main() {
    ...
    cin>>n;
    cout<<"Enter arrival, departure, charge needed for each EV\n";
    for(int i=0;i<n;i++) {
        int s,e,c;
    ...
    // calling ftime function for finding each car ending time
